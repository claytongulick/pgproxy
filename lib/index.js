    ...
    static _createPgFunctionSource(source, name, options) {
        ...
        ${functions.map(
            ...
            (fn) => (fn == name) ? '': `function ${fn}() {
                let return_value = plv8.execute('select ${options.schema}.pgproxy_${fn.toLowerCase()}(\\'' + JSON.stringify([...arguments]) + '\\'::json)');
                return return_value[0]['pgproxy_${fn.toLowerCase()}'];
            }`
        ).join('')}
